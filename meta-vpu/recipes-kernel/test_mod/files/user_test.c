...
int main() {
    ...
    write_value = 12;
    if ((bytes_write
         = pwrite(fd, &write_value, sizeof(write_value), EDU_FACT_CALC))
        == sizeof(write_value)) {
    ...
    if (pwrite(fd, &buffer_address, sizeof(buffer_address), EDU_DMA_SRC_ADDRESS)
    ...
    if (pwrite(fd, &buffer_address, sizeof(buffer_address), EDU_DMA_DST_ADDRESS)
        != sizeof(buffer_address)) {
        printf("Failed to set DMA destination address");
        return errno;
    }
